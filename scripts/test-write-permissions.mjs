...
test("C3h-1 已保护接口精确为二十五个写 handler", () => {
  assert.equal(protectedHandlers.size, 25);
...
test("待实施注册表精确为 7 个且均记录阶段", () => {
  assert.equal(pendingHandlers.size, 7);
...
test("二十五个已保护写 handler 使用精确的完整资源链权限组合", () => {
...
test("二十五个已保护写路由统一使用全权限助手且不回退 requireApiUser", () => {
...
test("二十五个已保护写 handler 权限失败后立即返回", () => {
